...
export default function AllSubscriptionsList({
  userId,
  subs,
  blogNameParam,
}: SubscribedBlogItemProps) {
  const [subscribedBlogs, setSubscribedBlogs] =
    useState<UserSubscription[]>(subs)
  ...
  return (
    <div class="flex flex-col space-y-2 lg:p-2 text-sm">
      {subscribedBlogs.map((sub) => {
        return (
          <div
            class={`flex flex-row justify-between hover:outline hover:outline-zinc-600/70
                    rounded-lg p-2 shadow-md transition:ease-in duration-100 space-x-1
                    ${blogNameParam === sub.companyName ? "bg-zinc-600" : ""}`}
          >
            <button
              onClick={() => {
                window.location.href = `/posts?blog=${sub.companyName}&page=1`
              }}
              class="w-full h-full"
            >
              <p
                class="rounded-lg hover:shadow-sm hover:shadow-black
            ...
            <div class="flex flex-row space-x-1">
              <button class="hover:bg-zinc-400 focus:bg-zinc-400 rounded-full">
                <a href={`${sub.blogLink}`}>

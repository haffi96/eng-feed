...
export class DevFeedStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const fetchUserToNotifyRule = new events.Rule(
      this,
      "fetchUserToNotifyRule",
      {
        schedule: events.Schedule.cron(cron_exp),
      }
    )
    fetchUserToNotifyRule.addTarget(
      new targets.LambdaFunction(fetchUserToNotify)
    )
    ...
    snsTopic.addSubscription(
      new snsSubscriptions.LambdaSubscription(sendEmailLambda)
    )

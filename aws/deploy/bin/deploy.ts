...
const defaultVpc = ec2.Vpc.fromLookup(stack, "default-vpc", { vpcId: "vpc-004c22d6b0586f89a" });

// Fetch security group from existing security group ID
const securityGroup = ec2.SecurityGroup.fromSecurityGroupId(stack, 'test-sg-group', 'sg-04a2f17075bbb4551');
...
// Create DB secrets
const masterUserSecret = new Secret(stack, "test-db-secret", {
...
const dbInstance = new rds.DatabaseInstance(stack, 'feed', {
...
// Create an SQS Queue
const queue = new sqs.Queue(stack, 'test-sqs-queue');

// Create an SNS Topic
const snsTopic = new sns.Topic(stack, 'test-sns-topic');


// Create a Lambda function for posting messages to SQS
const postsLambda = new lambda.Function(stack, 'PostsLambda', {
...
const sendEmailLambda = new lambda.Function(stack, 'SendEmailLambda', {
  runtime: lambda.Runtime.NODEJS_18_X,
...
const rule = new events.Rule(stack, 'PostsRule', {
